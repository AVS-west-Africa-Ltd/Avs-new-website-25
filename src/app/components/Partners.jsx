...
export default function Partners() {
    ...
    return (
        <div className="bg-neutral-200 text-3xl flex flex-col py-20 md:py-40 px-2 md:px-2 w-full space-y-4 md:space-y-10">
            <h2 className="font-khand font-medium text-neutral-400 text-center">Partners we work with</h2>
            <div className="flex items-center w-full mx-auto md:space-x-8 relative">
                <button className="w-1/12 text-xs md:text-sm flex p-4 text-right opacity-20 hover:opacity-100 transition duration-300 max-md:hidden" onClick={() => moveSlide('prev')}>
                    &#9668;
                </button>
                <div className="overflow-hidden w-10/12 max-md:m-auto flex relative" ref={containerRef}>
                    <motion.div
                    ...
                    >
                        {partners.map((partner, index) => (
                            <div key={`partner_${partner.alt}`} className="w-[150px] md:w-[220px] h-[50px] relative overflow-hidden m-auto">
                                <Image src={partner.src} alt={partner.alt} size="100vw" fill className="object-contain" />
                            </div>
                        ))}
                    </motion.div>
                </div>
                <button className="w-1/12 text-xs md:text-sm flex p-4 text-right opacity-20 hover:opacity-100 transition duration-300 max-md:hidden" onClick={() => moveSlide('next')}>
                    &#9658;
                </button>
            </div>
        </div>
