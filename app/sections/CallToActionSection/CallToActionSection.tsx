...
export const CallToActionSection = () => {
  const {
    ...
    queryKey: ['page', sanityPageConfig.footerPageId],
    queryFn: () => fetchPageData(sanityPageConfig.footerPageId),
  });
  ...
  const fetchPageData = async (pageId: string) => {
    ...
    return result?.contentBlocks?.[0]; // Extract the siteFooter data
  };
  ...
  const siteFooterData = pageData;

  if (isLoading ||!siteFooterData) {
    return (
      <>
        <footer className="w-full bg-[linear-gradient(180deg,#131211_0%,#1A1816_100%)] min-h-[448px] flex flex-col px-[58px] py-[92px] max-md:px-10 max-sm:px-5">
          ...
          <div className="flex flex-col items-center gap-3.5">
            ...
            <button className="text-[#EDE5E5] text-[15px] leading-[19.5px] tracking-[-0.3px] border px-2 py-1 md:px-4 md:py-2 rounded-[33px] border-[#EFF0F4] hover:bg-[#EFF0F4] hover:text-[#131211] transition-colors">
               <a href="https://www.linkedin.com/company/a-venture-studio-avs/" target="_blank">LinkedIn</a> 
              </button>
              <button className="text-[#EDE5E5] text-[15px] leading-[19.5px] tracking-[-0.3px] border px-2 py-1 md:px-4 md:py-2 rounded-[33px] border-[#EFF0F4] hover:bg-[#EFF0F4] hover:text-[#131211] transition-colors">
                Facebook
              </button>
...
{siteFooterData?.brandStatement && <PortableText value={siteFooterData.brandStatement} />}
          ...
          <div className="flex gap-[40px] max-md:flex-wrap max-sm:gap-10">
            <div className="flex gap-[44px] max-md:flex-wrap max-sm:gap-10">
              ...
              <nav aria-label="Legal navigation">
                ...
                <ul className="flex flex-col gap-[11px]">
                  {siteFooterData?.legalLinks?.map((link: any) => (
                    <li key={link._key}>
                      <Link
                        href={link.url}
                        className="text-white text-[15px] md:text-[14px] lg:text-[15px] leading-[19.5px] tracking-[-0.3px] hover:underline"
    ...
    className="text-white text-[15px] md:text-[14px] lg:text-[15px] leading-[19.5px] tracking-[-0.3px] hover:underline"
        ...
        <div className="flex flex-col items-center gap-3.5">
          ...
          {siteFooterData?.socialLinks?.map((social: any) => (
              <button
                key={social?._key}
...
//             <nav aria-label="Legal navigation">
//               {siteFooterData?.legalLinks?.length > 0 && (
//                 <h2 className="text-white text-[15px] leading-[19.5px] tracking-[-0.3px] mb-[11px]">
//                   Legal
//                 </h2>
//               )}
