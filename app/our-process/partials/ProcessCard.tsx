...
const ProcessCard = ({ step, isReversed }: { step: any; isReversed: boolean }) => {
    return (
        <motion.div
        ...
        >
            <Image
                ...
                src={step?.image ? urlFor(step.image).url() : '/assets/placeholder.png'}
                width={470}
                height={538}
                priority
            />
            <div className="flex flex-col w-full sm:w-[90%] md:w-[60%] lg:w-[444px] items-start gap-4 md:gap-6">
                <div className="w-full font-raleway font-bold text-[#93969F] text-3xl sm:text-4xl md:text-5xl">{step.stepNumber}</div>
